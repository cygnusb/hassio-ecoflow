...
async def async_setup_entry(
    ...
    if is_power_station(client.product):
        entities.extend(
            [
                CurrentEntity(
                    client, client.inverter, "ac_in_current", "AC input current"
                ),
                CurrentEntity(
                    client, client.inverter, "ac_out_current", "AC output current"
                ),
                EnergyEntity(client, client.pd, "mppt_in_energy", "MPPT input energy"),
                EnergySumEntity(
                    client, "in_energy", ["ac", "car", "mppt"], "Total input energy"
                ),
                EnergySumEntity(
                    client, "out_energy", ["ac", "car"], "Total output energy"
                ),
                FanEntity(client, client.inverter, "fan_state", "Fan"),
                FrequencyEntity(
                    client, client.inverter, "ac_in_freq", "AC input frequency"
                ),
                FrequencyEntity(
                ...
                VoltageEntity(
                    client, client.inverter, "ac_out_voltage", "AC output voltage"
                ),
                ...
                WattsEntity(
                    client, client.inverter, "ac_out_power", "AC output", real=False
                ),
                WattsEntity(client, client.pd, "usb_out1_power", "USB-A left output"),
        ...
        if is_delta(client.product):
            ...
            entities.extend(
                [
                    CurrentEntity(
                        client, client.mppt, "dc_in_current", "DC input current"
                    ),
                    CyclesEntity(
                        client, bms[0], "battery_cycles", "Main battery cycles", 0
                    ),
                    RemainEntity(
                        client, client.ems, "battery_remain_charge", "Remain charge"
                    ),
                    RemainEntity(
                        client,
                        client.ems,
                        "battery_remain_discharge",
                        "Remain discharge",
                    ),
                    SingleLevelEntity(
                    ...
                    TempEntity(
                        client, client.inverter, "ac_out_temp", "AC temperature"
                    ),
                    TempEntity(
                        client, bms[0], "battery_temp", "Main battery temperature", 0
                    ),
                    TempEntity(
                        client, client.mppt, "dc_in_temp", "DC input temperature"
                    ),
                    TempEntity(
                        client, client.mppt, "dc24_temp", "DC output temperature"
                    ),
                    TempEntity(
                        client, client.pd, "typec_out1_temp", "USB-C left temperature"
                    ),
                    TempEntity(
                        client, client.pd, "typec_out2_temp", "USB-C right temperature"
                    ),
                    VoltageEntity(
                        client, client.mppt, "dc_in_voltage", "DC input voltage"
                    ),
                    WattsEntity(client, client.inverter, "ac_in_power", "AC input"),
                    WattsEntity(
                        client, client.mppt, "dc_in_power", "DC input", real=True
                    ),
                    WattsEntity(
                        client,
                        client.mppt,
                    ...
                    WattsEntity(client, client.mppt, "car_out_power", "Car output"),
                ]
            )
            if is_delta_mini(client.product):
                entities.extend(
                    [
                        WattsEntity(
                            client, client.pd, "usbqc_out1_power", "USB-Fast output"
                        ),
                        WattsEntity(
                            client, client.pd, "typec_out1_power", "USB-C output"
                        ),
                    ]
                )
            else:
                entities.extend(
                    [
                        ...
                        WattsEntity(
                            client, client.pd, "typec_out1_power", "USB-C left output"
                        ),
                        WattsEntity(
                            client, client.pd, "typec_out2_power", "USB-C right output"
                        ),
                    ]
                )
            if is_delta_pro(client.product):
                entities.extend(
                    [
                        WattsEntity(
                            client, client.mppt, "anderson_out_power", "Anderson output"
                        ),
                    ]
                )
        if is_river(client.product):
            ...
            entities.extend(
                [
                    CurrentEntity(
                        client, client.inverter, "dc_in_current", "DC input current"
                    ),
                    CyclesEntity(
                        client, client.ems, "battery_cycles", "Main battery cycles"
                    ),
                    CyclesEntity(
                        client, extra, "battery_cycles", "Extra battery cycles", 1
                    ),
                    SingleLevelEntity(
                        client, client.ems, "battery_main_level", "Main battery"
                    ),
                    SingleLevelEntity(
                        client, extra, "battery_level", "Extra battery", 1
                    ),
                    TempEntity(
                        client, client.inverter, "ac_in_temp", "AC input temperature"
                    ),
                    TempEntity(
                        client, client.inverter, "ac_out_temp", "AC output temperature"
                    ),
                    TempEntity(
                        client,
                        client.ems,
                    ...
                    TempEntity(
                        client, extra, "battery_temp", "Extra battery temperature", 1
                    ),
                    TempEntity(
                        client, client.pd, "car_out_temp", "DC output temperature"
                    ),
                    TempEntity(
                        client, client.pd, "typec_out1_temp", "USB-C temperature"
                    ),
                    VoltageEntity(
                        client, client.inverter, "dc_in_voltage", "DC input voltage"
                    ),
                    ...
                    WattsEntity(client, client.pd, "typec_out1_power", "USB-C output"),
                ]
            )
...
class EnergySumEntity(EnergyEntity):
    def __init__(
        self, client: HassioEcoFlowClient, key: str, keys: list[str], name: str
    ):
    ...
    def _on_updated(self, data: dict[str, Any]):
        values = {
            key[: -self._suffix_len]: data[key] for key in data if key in self._keys
        }
...
class SingleLevelEntity(LevelEntity):
    def _on_updated(self, data: dict[str, Any]):
        ...
        if "battery_capacity_remain" in data:
            self._attr_extra_state_attributes["capacity_remain"] = data[
                "battery_capacity_remain"
            ]
        if "battery_capacity_full" in data:
            self._attr_extra_state_attributes["capacity_full"] = data[
...
class WattsEntity(BaseEntity):
    ...
    def _on_updated(self, data: dict[str, Any]):
        key = self._key[:-5]
        if (
            self._real is not False
            and f"{key}current" in data
            and f"{key}voltage" in data
        ):
            self._attr_native_value = data[f"{key}current"] * data[f"{key}voltage"]
            if self._real is not True:
                self._attr_native_value = round(self._attr_native_value, self._real)
                if self._real == 0:
